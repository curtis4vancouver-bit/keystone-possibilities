...
# Legacy Windows encoding overrides removed to prevent Python 3.14 I/O closed file errors.
...
# ── Token Management ──────────────────────────────────────────────
PRIMARY_TOKEN = os.path.join(SCRIPT_DIR, "youtube_token.json")
OAC_TOKEN = os.path.join(SCRIPT_DIR, "youtube_token_oac.json")
_yt_cache = {}

CHANNELS = {
    "possibilities": {"id": "UCu8gdU_R8XE2RvcttGa3drg", "token": "primary", "niche": "construction", "handle": "@KeystonePossibilities"},
    "protocols":     {"id": "UCxURlqMNhAtxUTpdXmlOYaw", "token": "primary", "niche": "health",       "handle": "@KeystoneProtocols"},
    "oac":           {"id": "UCMn1f9DTF_iybKmv5WlTm9Q", "token": "oac",     "niche": "music",        "handle": "@KeyStoneRecomposition"},
}
...
def _get_youtube(channel_key):
    """Get YouTube API client for the specified channel."""
    ch = CHANNELS.get(channel_key)
    if not ch:
        return None
    token_key = ch["token"]
    if token_key not in _yt_cache:
        token_path = OAC_TOKEN if token_key == "oac" else PRIMARY_TOKEN
        if not os.path.exists(token_path):
            return None
        creds = Credentials.from_authorized_user_file(token_path)
        _yt_cache[token_key] = build("youtube", "v3", credentials=creds)
    return _yt_cache[token_key]
