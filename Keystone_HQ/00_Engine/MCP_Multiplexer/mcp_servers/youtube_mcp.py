...
def get_manager(channel: str = "oac") -> YouTubeAPIManager:
    ...
    if channel not in _managers:
        ...
        # If the channel-specific token doesn't exist, try falling back to legacy youtube_token.json
        ...
        _managers[channel] = YouTubeAPIManager(token_file=token_path)
...
@mcp.tool()
def update_video_metadata(video_id: str, title: str = None, description: str = None, tags: list = None, category_id: str = None) -> str:
    """
    ...
    """
    try:
        response = manager.update_video_metadata(video_id, title, description, tags, category_id)
        if response:
