...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.action === "SET_PROJECT_FOLDER") {
    ...
    if (msg.payload.outputDirectory !== undefined) updateObj.outputDirectory = msg.payload.outputDirectory;
    
    chrome.storage.local.set(updateObj, () => {
      console.log(`[Background] Project folder set to: ${msg.payload.folderName}, Output: ${msg.payload.outputDirectory || '(default)'}`);
...
chrome.downloads.onDeterminingFilename.addListener((downloadItem, suggest) => {
  ...
  chrome.storage.local.get({ 
    ...
    const globalOutputDir = data.outputDirectory;
    ...
    chrome.storage.local.set({ downloadQueue: queue }, () => {
      // Use per-entry override if present, otherwise use global output directory
      let prefix = entry.outputDirectory || globalOutputDir || '../Desktop/';

      // Normalize: ensure trailing slash, handle dot notation for Downloads itself
      if (prefix === '.' || prefix === './') {
        prefix = '';
