...
const agentsConfigFile = path.resolve(__dirname, "../MCP_Multiplexer/agents.json");
...
async function testHealth() {
  ...
  const multiplexerDir = path.resolve(__dirname, "../MCP_Multiplexer");
  args = args.map(arg => {
    if (typeof arg === "string" && (arg.startsWith("../") || arg.startsWith("./") || arg.startsWith("agents/"))) {
      return path.resolve(multiplexerDir, arg);
    }
    return arg;
  });
  
  if (typeof command === "string" && (command.startsWith("../") || command.startsWith("./"))) {
    command = path.resolve(multiplexerDir, command);
  }
  ...
  const workspaceRoot = path.resolve(multiplexerDir, "..");
