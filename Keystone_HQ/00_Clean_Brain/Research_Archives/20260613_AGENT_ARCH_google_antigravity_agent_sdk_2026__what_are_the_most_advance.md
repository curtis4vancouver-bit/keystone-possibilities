...
Automated [[davinci-resolve-mcp/docs/SKILL|Skill]] Creation and Progressive Disclosure
...
A "[[davinci-resolve-mcp/docs/SKILL|Skill]]" within the Antigravity framework acts as an on-demand specialist knowledge package. Structurally, a [[davinci-resolve-mcp/docs/SKILL|skill]] is a dedicated directory containing a core [[davinci-resolve-mcp/docs/SKILL|SKILL]].md file, alongside optional helper scripts, templates, and reference implementations, all housed within the .[[AGENTS|agents]]/skills/ directory of the project.   

When a conversation begins, the Antigravity agent is not fed the contents of every [[davinci-resolve-mcp/docs/SKILL|skill]] file. Instead, it is provided solely with a lightweight [[wiki/index|index]] displaying the names and brief descriptions of the available skills. If the agent determines that a specific task requires domain expertise—for instance, evaluating a complex shadow deployment for the health portal—it dynamically activates the relevant [[davinci-resolve-mcp/docs/SKILL|skill]], instructing the system to load the full contents of that specific [[davinci-resolve-mcp/docs/SKILL|SKILL]].md file directly into its active context window. This ensures the agent maintains a lean, hyper-focused context window tailored precisely to the task at hand.   

The pinnacle of autonomous evolution is reached when the agent transcends merely utilizing pre-written skills and begins authoring its own. Drawing inspiration from the open-source antigravity-self-evolving-reviews methodology, the agent can be programmed to utilize "meta-prompts". These are sophisticated instructions commanding the agent to scan its own repository configurations, analyze current dependency trees (e.g., parsing a package.json file), and evaluate its history of past deployment errors retrieved from the FTS5 memory [[wiki/index|index]].   

Based on this comprehensive self-analysis, the agent autonomously generates a hyper-specific set of operational rules, drafts a new set of programmatic procedures, runs the dual-model stress-test validation, and finally writes a brand new [[davinci-resolve-mcp/docs/SKILL|SKILL]].md file to the local disk using its own file-writing capabilities. Through this mechanism, a novel solution improvised by the agent on a Monday is permanently codified into an executable, on-demand specialist [[davinci-resolve-mcp/docs/SKILL|skill]] by Tuesday. This is the mechanism by which an autonomous system organically expands its own capabilities without waiting for human software updates.   
...
When an autonomous system reaches a [[STATE|state]] of maturity where it dynamically spawns multiple subagents to handle tasks concurrently, the physical [[Limitations|limitations]] of the filesystem become a significant bottleneck. If the media agent attempts to modify a global configuration file while the construction agent is simultaneously refactoring the database schema, file collisions, corruptions, and irrecoverable Git merge conflicts are statistically guaranteed.
