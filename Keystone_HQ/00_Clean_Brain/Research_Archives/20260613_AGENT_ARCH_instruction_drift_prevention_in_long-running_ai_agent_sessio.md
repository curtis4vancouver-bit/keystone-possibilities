...
When compaction is required, OpenCode first executes a local pruning process. It calculates if pruning can free up more than 20,000 tokens; if not, it bypasses the step. Crucially, OpenCode maintains a massive "Protection Cushion": it explicitly protects the most recent 40,000 tokens, the full contents of the last two user turns, and all "[[davinci-resolve-mcp/docs/SKILL|skill]]" type tool outputs that contain operational system instructions, ensuring core [[DIRECTIVES|directives]] are never pruned.   
