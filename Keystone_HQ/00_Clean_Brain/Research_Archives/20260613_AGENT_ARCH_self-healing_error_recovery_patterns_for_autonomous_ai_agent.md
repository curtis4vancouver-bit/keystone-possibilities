...
Advanced Autonomous Agent [[ARCHITECTURE|Architecture]]: Self-Healing Error Recovery and Implementation Patterns for 2026
...
Servers packaged with MSIX package [[Brand_Constitution/protocol/IDENTITY|identity]] are registered automatically upon installation and benefit fully from this containment. Conversely, standalone bundles lacking package [[Brand_Constitution/protocol/IDENTITY|identity]] cannot run in the contained process and are blocked from ODR discovery unless a user explicitly overrides the protections via Windows Settings.   
...
JavaScript
import { Client } from '@modelcontextprotocol/sdk/client/[[wiki/index|index]].js';
