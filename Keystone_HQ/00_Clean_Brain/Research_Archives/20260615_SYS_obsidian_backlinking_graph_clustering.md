...
A fundamental algorithmic challenge in lexical automation is avoiding the destructive modification of the user's data schema through [[wiki/index|index]] invalidation. When programmatic replacements alter the byte length of a string—for instance, expanding the word "Concept" into the markup `Concept`—all subsequent character [[wiki/index|index]] positions in the document are shifted forward. If a parser calculates the start and end positions of all keyword instances in a single pass and then attempts a sequential, forward-iterating replacement, the target indices for the second match will be invalidated by the string expansion of the first match, resulting in corrupted output. 
...
4. **Exact Canonical Matching**: The system verifies if the string matches an approved [[master|master]] taxonomy list in the current ontology registry.
