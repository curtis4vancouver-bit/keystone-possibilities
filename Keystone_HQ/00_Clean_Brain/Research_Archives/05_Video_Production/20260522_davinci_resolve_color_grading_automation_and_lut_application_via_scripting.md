...
def automate_node_grading(timeline_item, lut_path):
    ...
    # Set LUT (must strictly use 1-based [[wiki/index|index]]) 
...
When supplying the lut_path string to the SetLUT() function, the API accepts either an absolute path or a relative path based on Resolve's [[master|master]] LUT directories. Managing these paths in a distributed system requires knowing the exact system-level directory locations.   
...
By meticulously executing this technical [[ARCHITECTURE|architecture]], the Keystone Sovereign entity can entirely decouple the color grading and rendering processes from human operation, achieving instantaneous, infinite-scale post-production across its entire corporate enterprise.
