...
When utilizing the direct Vertex API or proxy wrappers, this is achieved by supplying an array of previously generated or uploaded mediaIds or Google Cloud Storage URIs. Up to three distinct reference images can be provided per generation request. By injecting these exact same three mediaIds into all 70 prompts dispatched to the Cloud Task queue, the agent enforces absolute character [[Brand_Constitution/protocol/IDENTITY|identity]], facial geometry, and stylistic consistency across the entire batch sequence, ensuring the AI physician looks identical in scene 1 and scene 70.   
