...
To replace the handshake, the specification introduces the server/discover remote procedure call. Servers must implement this RPC to advertise supported protocol versions, [[Brand_Constitution/protocol/IDENTITY|identity]] markers, and capabilities before a client initiates intensive tool calls. The JSON-RPC request for discovery carries no body parameters outside of the _meta object.   
...
// Initialize the application with metadata [[Brand_Constitution/protocol/IDENTITY|identity]]
...
Gateway Configuration and Workload [[Brand_Constitution/protocol/IDENTITY|Identity]] Federation
