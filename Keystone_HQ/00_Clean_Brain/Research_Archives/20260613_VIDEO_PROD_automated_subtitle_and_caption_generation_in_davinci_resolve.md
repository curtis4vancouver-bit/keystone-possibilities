...
1. DaVinci Resolve Python API [[ARCHITECTURE|Architecture]] and Environment Initialization
...
def calculate_exact_frame(milliseconds: float, fps: float, start_tc_frame: int):
    ...
    # Floor is used to ensure the frame [[wiki/index|index]] captures the start of the temporal block
...
trackIndex	Integer	2 (e.g., Video Track 2)	The 1-based [[wiki/index|index]] of the target track. Crucially, this must be a standard video track, not a subtitle track.
...
def apply_textplus_styles_and_dialogue(timeline, track_index, subtitle_data_list):
    ...
    for [[wiki/index|index]], clip in enumerate(clips):
        # Retrieve the specific text string for this clip
        dialogue_text = subtitle_data_list[[wiki/index|index]]['text']
...
def extract_srt_from_timeline(timeline_obj, fps: float, output_path: str):
    ...
    """
    # 1 is the default track [[wiki/index|index]] for ST1
