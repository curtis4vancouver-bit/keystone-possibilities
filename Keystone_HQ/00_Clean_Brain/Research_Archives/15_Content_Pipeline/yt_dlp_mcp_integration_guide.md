...
The Model Context Protocol is structured as a client-host-server [[ARCHITECTURE|architecture]] designed to maintain high degrees of isolation while enabling fluid data exchange. The host process—typically an AI application such as [[CLAUDE|Claude]] Desktop or an integrated development environment—serves as the primary orchestrator that creates and manages multiple client instances. Each client maintains a dedicated, one-to-one connection with a specific MCP server, ensuring that data and permissions do not leak across different service boundaries.
