# Custom Antigravity [[davinci-resolve-mcp/docs/SKILL|Skill]] Creation & Multi-File [[ARCHITECTURE|Architecture]] (2026)
## Developer Reference Blueprint for Dynamic [[davinci-resolve-mcp/docs/SKILL|Skill]] Registries, AST Sandboxing, and Multi-Agent Tool Discovery
...
### 1.2 The `[[davinci-resolve-mcp/docs/SKILL|SKILL]].md` Specification
Every [[davinci-resolve-mcp/docs/SKILL|skill]] must contain a `[[davinci-resolve-mcp/docs/SKILL|SKILL]].md` at its root with valid YAML frontmatter specifying its identifier, capabilities, and conditional activation predicates.
...
When writing complex tools inside a custom [[davinci-resolve-mcp/docs/SKILL|skill]], tools must be able to import shared helper scripts from the [[davinci-resolve-mcp/docs/SKILL|skill]]'s local directory without polluting the host system's standard Python `sys.path`.

The following script demonstrates how a dynamically executed [[davinci-resolve-mcp/docs/SKILL|skill]] tool securely resolves and loads local sub-modules using relative path insertion.
...
If your custom [[davinci-resolve-mcp/docs/SKILL|skill]] connects to a FastMCP server or generates tools, the safety policy must declare narrow, prefix-matched scopes:
...
## 5. Programmatic Multi-File [[davinci-resolve-mcp/docs/SKILL|Skill]] Ingestion Loop

To test and execute a custom [[davinci-resolve-mcp/docs/SKILL|skill]], we use the `LocalAgentConfig` to point to our dynamic [[davinci-resolve-mcp/docs/SKILL|skill]] directory. The following script demonstrates loading a custom [[davinci-resolve-mcp/docs/SKILL|skill]], validating its internal modules through our AST sandbox, registering safety controls, and running an agent loop.
...
Follow these steps to deploy and register a custom [[davinci-resolve-mcp/docs/SKILL|skill]] securely:

- [ ] **Establish `[[davinci-resolve-mcp/docs/SKILL|SKILL]].md` Metadata:** Define a valid YAML frontmatter containing `name`, `description`, `version`, and narrow activation `predicates`.
...
- [ ] **Apply safety policy constraints:** Grant explicit grants in your agent's config using `policy.grant("mcp", "your-[[davinci-resolve-mcp/docs/SKILL|skill]]/*")` rather than broad wildcards.
