...
In the Google Antigravity [[ARCHITECTURE|architecture]], there are two complementary ways to expand an agent's capability:

| Extension Type | Execution Mechanism | Primary Use Case |
|---|---|---|
| **Skills (`[[davinci-resolve-mcp/docs/SKILL|SKILL]].md`)** | Code-driven markdown instructions and AST compilation executed in the workspace context. | Custom domain-specific workflows, code generation guidelines, and structured system configurations (e.g., Wolverine Stack setups). |
...
4. **PostgreSQL MCP (`postgres`)**: Enables secure schema discovery, [[wiki/index|index]] tuning, and direct query execution inside local or remote databases.
...
## 4. Custom [[davinci-resolve-mcp/docs/SKILL|Skill]] Server Creation: The FastMCP Python Template
...
1. **Path Sandboxing**: Never allow an MCP tool or compiled [[davinci-resolve-mcp/docs/SKILL|skill]] to resolve arbitrary directories. Always bind operations to a restricted directory path:
   ```python
