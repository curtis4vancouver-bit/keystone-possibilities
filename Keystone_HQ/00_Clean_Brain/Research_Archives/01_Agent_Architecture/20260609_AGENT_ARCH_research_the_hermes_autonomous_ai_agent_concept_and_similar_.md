...
Production [[ARCHITECTURE|Architecture]] of Self-Improving Autonomous AI [[AGENTS|Agents]] (May 2026)
...
Stable Tier: Immutable agent [[Brand_Constitution/protocol/IDENTITY|identity]] (loaded from SOUL.md), core operational [[DIRECTIVES|directives]], and indexed [[davinci-resolve-mcp/docs/SKILL|skill]] availability.   

Context Tier: Attached project guidelines, including [[AGENTS|AGENTS]].md, .hermes.md, [[CLAUDE|CLAUDE]].md, or .cursorrules.   
...
Rather than overloading the main system prompt with exhaustive instructions for every possible operational task, [[AGENTS|agents]] load knowledge through progressive disclosure. A "[[davinci-resolve-mcp/docs/SKILL|Skill]]" is packaged as a portable folder containing a [[davinci-resolve-mcp/docs/SKILL|SKILL]].md file. The specification defines strict constraints: the name must use lowercase letters, numbers, and hyphens (maximum 64 characters), and the description must describe both what the [[davinci-resolve-mcp/docs/SKILL|skill]] does and when to use it, avoiding XML angle brackets in the frontmatter to prevent prompt injection.   
...
Activation: When a user request matches the description, the agent executes a tool call to read the full [[davinci-resolve-mcp/docs/SKILL|SKILL]].md file, incorporating the procedural knowledge into its active context.   

Execution: The agent follows the instructions, optionally executing bundled Python or Bash scripts located within the [[davinci-resolve-mcp/docs/SKILL|skill]]'s scripts/ subdirectory.   

By version-controlling these [[davinci-resolve-mcp/docs/SKILL|skill]] folders, an enterprise can standardize procedural knowledge across its entire workforce. When a human expert optimizes the pipeline for uploading health content, they simply update the health-publishing/[[davinci-resolve-mcp/docs/SKILL|SKILL]].md file in the central repository. Instantly, any skills-compatible agent across the organization inherits the optimized procedure, allowing cross-product reuse and auditable workflows.   
...
The system will run nightly "Compound Engineering" bash loops across all domains. Using the agentskills.io standard, specific [[DIRECTIVES|directives]] for YouTube video tagging or site safety compliance are stored as individual [[davinci-resolve-mcp/docs/SKILL|SKILL]].md files.   

If a YouTube API endpoint changes and a video upload fails, or if a construction vendor updates their invoice formatting, the execution loop will log the failure. During the 10:30 PM Compound Review phase, the agent will analyze the failure, deduce the necessary correction, update the local norm in the repository memory ([[AGENTS|AGENTS]].md), and rewrite the specific youtube-upload or invoice-processing [[davinci-resolve-mcp/docs/SKILL|skill]]. At 11:00 PM, the Auto-Compound script will pull these fresh instructions, execute the stalled task, and push a completed Pull Request.   
