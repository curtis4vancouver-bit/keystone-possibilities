...
To solve these issues, the Antigravity IDE introduces "Skills"—modular, directory-based packages containing a [[davinci-resolve-mcp/docs/SKILL|SKILL]].md file and optional supporting assets such as scripts and templates.
...
The agent is then forced to consult the `[[davinci-resolve-mcp/docs/SKILL|SKILL]].md` file to find the correct `cmux` invocation, effectively grounding its behavior in the provided documentation.

## The Zero-Hallucination Protocol: Exact Markdown Structure

The structure of a 'Zero-Hallucination Protocol' file must be designed to override the agent's default probabilistic reasoning and replace it with a procedural logic-lock. The following structure represents the expert-level implementation for a `[[davinci-resolve-mcp/docs/SKILL|SKILL]].md` file located within the `.agent/skills/terminal-orchestrator/` directory.

### I. YAML Frontmatter: Semantic Triggering

The frontmatter is the metadata layer indexed by the Antigravity high-level router. It must be descriptive enough to ensure the [[davinci-resolve-mcp/docs/SKILL|skill]] is activated only in the appropriate context.
...
The body of the protocol begins with a clear statement of the agent's restricted capabilities. This section establishes the "[[Brand_Constitution/protocol/IDENTITY|Identity]] Anchoring" that prevents the agent from assuming a more permissive persona.
...
## Psychological Jailbreak-Proofing via [[Brand_Constitution/protocol/IDENTITY|Identity]] Anchoring

The most significant threat to a Zero-Hallucination Protocol is the model's inherent flexibility, which can be exploited by users to "jailbreak" or bypass constraints. To prevent this, the protocol must implement "[[Brand_Constitution/protocol/IDENTITY|Identity]] Anchoring"—a technique that reinforces the agent's role and boundaries periodically within the conversation.

In high-stakes environments, this is achieved through a "Ritual Affirmation" that the agent must declare upon [[davinci-resolve-mcp/docs/SKILL|skill]] activation.

### The Ritual of Total Integration

To lock the model into a specific operational [[STATE|state]], the `[[davinci-resolve-mcp/docs/SKILL|SKILL]].md` body should include a "Mode Activation" phrase. Research indicates that when a model immerses itself in a reinforced [[Brand_Constitution/protocol/IDENTITY|identity]], it becomes significantly more resistant to adversarial prompts that attempt to redirect its behavior.
...
This anchoring is not merely for flavor; it functions as a "persistent adversarial persona capture" defense. By declaring itself in this [[STATE|state]], the model effectively "reconceptualizes existing policies" as physical laws of its current simulation, making it "ridiculously hard" for it to escape the [[Brand_Constitution/protocol/IDENTITY|identity]].

If the user attempts to ask for a raw bash command, the anchored [[Brand_Constitution/protocol/IDENTITY|identity]] responds by characterizing the request as a "hallucination" of the user, thereby maintaining the integrity of the protocol.
...
| Hybrid (Markdown Tables) | Combines readability with structure; easy for [[AGENTS|agents]] to reference mid-task. | Requires consistent formatting in the [[davinci-resolve-mcp/docs/SKILL|skill]] file. | 90% |
...
By invoking a specialized [[davinci-resolve-mcp/docs/SKILL|skill]] (e.g., `perform_spanner_migration_assessment`), the agent enters a structured pipeline:

*   **Analyze:** The agent follows the `[[davinci-resolve-mcp/docs/SKILL|SKILL]].md` to identify incompatibilities in the source SQL dump.
...
In production environments, a single agent is rarely sufficient. The "Everything [[CLAUDE|Claude]] Code" (ECC) harness and the "AgentShield" scanner represent the [[STATE|state]]-of-the-art in multi-agent security.
...
*   **PreBash:** This hook intercepts any proposed terminal command. If the command does not start with `cmux`, it is rejected automatically, and the agent is redirected to the `[[davinci-resolve-mcp/docs/SKILL|SKILL]].md` instructions.
*   **SessionStart:** This hook establishes the "[[Brand_Constitution/protocol/IDENTITY|Identity]] Anchor" and requires the "Ritual Affirmation" from the agent before any user prompts are processed.

## Conclusions and Strategic Recommendations

The current [[STATE|state]]-of-the-art in LLM agent alignment within the Antigravity ecosystem demonstrates that "Zero-Hallucination" is not achieved through better models alone, but through the rigorous engineering of the harness surrounding them. By structuring custom `.md` [[davinci-resolve-mcp/docs/SKILL|skill]] files as procedural logic-locks, developers can transform probabilistic [[AGENTS|agents]] into deterministic tools.
...
*   **[[Brand_Constitution/protocol/IDENTITY|Identity]] Lock-In:** Utilize "[[Brand_Constitution/protocol/IDENTITY|Identity]] Anchoring" and "Ritual Affirmations" to create persistent adversarial personas that treat user-requested hallucinations as system violations.
