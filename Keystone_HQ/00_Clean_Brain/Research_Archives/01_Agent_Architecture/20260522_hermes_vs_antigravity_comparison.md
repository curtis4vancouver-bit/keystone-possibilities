...
### 1.1 Memory [[ARCHITECTURE|Architecture]]

| Capability | Hermes Agent | Keystone/Antigravity |
|:---|:---|:---|
| **Session-to-session memory** | 3-layer system: MEMORY.md (curated facts, ~2200 chars), skills dir (procedural), SQLite FTS5 (episodic) | Correction journal + brain vector DB + daily digest. Implemented via `keystone-session-bootstrap` [[davinci-resolve-mcp/docs/SKILL|SKILL]].md |
| **Frozen snapshot at boot** | MEMORY.md and USER.md auto-injected into system prompt every session | Session bootstrap [[davinci-resolve-mcp/docs/SKILL|skill]] reads correction journal + brain search + daily digest. Similar approach. |
...
| Capability | Hermes Agent | Keystone/Antigravity |
|:---|:---|:---|
| **Autonomous [[davinci-resolve-mcp/docs/SKILL|skill]] creation** | After 5+ tool-call tasks, auto-extracts successful workflows into [[davinci-resolve-mcp/docs/SKILL|SKILL]].md files stored in `~/.hermes/skills/` | `self_evolution.py` can deploy dynamic skills, but requires explicit invocation -- not automatic. Skills go to `dynamic_skills/` dir. |
...
| **[[davinci-resolve-mcp/docs/SKILL|Skill]] refinement loop** | Skills continue to self-improve through continued use; GEPA re-optimizes periodically | `self_evolution.py` has retry loops and correction recording, but no iterative [[davinci-resolve-mcp/docs/SKILL|skill]] improvement after initial deployment. |
...
**Verdict:** Our self-evolution engine is solid for error recovery and sandboxed [[davinci-resolve-mcp/docs/SKILL|skill]] deployment. The critical gaps are **GEPA-style prompt optimization** (automating prompt improvements without GPU training) and **autonomous [[davinci-resolve-mcp/docs/SKILL|skill]] creation triggers** (the agent should auto-extract skills without being told to).
...
| **Error fingerprinting** | Not explicitly documented as fingerprinting, but [[davinci-resolve-mcp/docs/SKILL|skill]] updates encode lessons learned | `_compute_error_fingerprint()` using SHA-256 hashing of error type + context + traceback. **We are ahead here.** |
...
| Capability | Hermes Agent | Keystone/Antigravity |
|:---|:---|:---|
| **Auto-discovery** | Scans CWD for `.hermes.md` -> `[[AGENTS|AGENTS]].md` -> `CLAUDE.md` -> `.cursorrules` | Antigravity uses GEMINI.md files and [[davinci-resolve-mcp/docs/SKILL|skill]] YAML frontmatter. **Comparable.** |
...
| **SOUL.md personality** | Separate file defines agent personality/[[Brand_Constitution/protocol/IDENTITY|identity]] globally | Our skills define behavioral rules but no dedicated personality file. **Minor gap.** |
...
| **Community [[davinci-resolve-mcp/docs/SKILL|skill]] sharing** | agentskills.io open standard, hermeshub, skilldock.io marketplaces | No community marketplace. Skills are local to our config. **GAP.** |

**Verdict:** Our MCP architecture is more extensible than Hermes' tool registry. However, we lack **model fallback/pooling** and **community [[davinci-resolve-mcp/docs/SKILL|skill]] sharing**.
...
| **Autonomous [[davinci-resolve-mcp/docs/SKILL|Skill]] Trigger** | After 5+ tool calls on a complex task, automatically extracts and saves a reusable [[davinci-resolve-mcp/docs/SKILL|skill]] without user prompting. | HIGH -- Our `self_evolution.py` creates skills but only when explicitly called. The auto-trigger is the key difference. |
...
1. **Autonomous [[davinci-resolve-mcp/docs/SKILL|Skill]] Extraction Trigger**
   - Modify the agent loop to detect when a task used 5+ tool calls and completed successfully
   - Auto-generate a [[davinci-resolve-mcp/docs/SKILL|SKILL]].md capturing the workflow
   - Save to `dynamic_skills/` and sync to multiplexer
   - Implementation: Add a post-task hook in the session bootstrap that counts tool invocations and triggers `workflow-[[davinci-resolve-mcp/docs/SKILL|skill]]-creator` [[davinci-resolve-mcp/docs/SKILL|skill]] automatically
...
3. **GEPA-Lite Prompt Optimizer**
   - Implement a simplified version of GEPA for our [[davinci-resolve-mcp/docs/SKILL|skill]] files
...
7. **Trajectory Capture for Audit**
   ...
   - Use for: debugging, [[davinci-resolve-mcp/docs/SKILL|skill]] extraction training data, performance analytics
...
2. **Structured Error Fingerprinting:** SHA-256 fingerprinting with known-fix lookup in the correction journal is more sophisticated than Hermes' [[davinci-resolve-mcp/docs/SKILL|skill]]-based error encoding.
...
5. **Security Sandbox:** AST-level code validation before [[davinci-resolve-mcp/docs/SKILL|skill]] deployment. Hermes has community-contributed hardening patches but no formal AST sandbox.
...
## 4. Recommended Implementation: Autonomous [[davinci-resolve-mcp/docs/SKILL|Skill]] Extraction

Here is a concrete design for the highest-priority gap -- automatic [[davinci-resolve-mcp/docs/SKILL|skill]] extraction:
...
| Self-Evolution | 60% parity | Add GEPA-lite + auto [[davinci-resolve-mcp/docs/SKILL|skill]] triggers |
...
**Bottom line:** Hermes' three killer features we lack are (1) multi-platform messaging gateway, (2) autonomous [[davinci-resolve-mcp/docs/SKILL|skill]] extraction triggers, and (3) GEPA prompt evolution. Everything else we either match or exceed. Closing these three gaps would make our Keystone system definitively superior.
