...
> **Date:** June 14, 2026  
> **Author:** Antigravity [[master|Master]] Brain Fleet Swarm  
...
3.  This plugin includes native support for the **Model Context Protocol (MCP)**, allowing MCP-compliant [[AGENTS|agents]] to [[wiki/index|index]] and search notes programmatically.
...
*   **Key Concept:** It starts with a blank slate. If it encounters a task it cannot do, it writes a custom script, tests it, and saves it as a static `[[davinci-resolve-mcp/docs/SKILL|SKILL]].md` instruction folder inside its directory, conforming to the [agentskills.io](https://agentskills.io) standard. It also manages a lightweight, FTS5-enabled SQLite database for session memory.
*   **Transplant Target:** Look at `skills/` directory for [[davinci-resolve-mcp/docs/SKILL|skill]]-parsing routines. Hermes implements **progressive disclosure**, reading only [[davinci-resolve-mcp/docs/SKILL|skill]] names/descriptions at boot to minimize token count. It only reads the full script and markdown instructions when the user request triggers that specific [[davinci-resolve-mcp/docs/SKILL|skill]].
...
*   **Core METAPHOR:** [[davinci-resolve-mcp/docs/SKILL|Skill]] Crystallization.
...
*   **Recommended Hardware Configuration**:
    ...
    *   **Compute [[ARCHITECTURE|Architecture]]**: Stick to NVIDIA GPUs. Ollama, llama.cpp, and vLLM rely on native CUDA for Windows acceleration.
...
5.  **Transplant [[davinci-resolve-mcp/docs/SKILL|Skill]] Crystallizer**: Add the crystallization logic to `self_learning.py` so that local executions compile successful workflows into markdown files within `Skill_Vault/`, keeping the local assistant smart.
