...
**Applicability:** Keystone Sovereign [[master|Master]] Brain (sqlite-vec backed vector DB with ~1,158 chunks across 57 sources)
...
Target **400-512 tokens** per chunk for our technical markdown documents. Given that our brain currently holds ~1,158 chunks across 57 sources (averaging ~20 chunks per document), this size maintains manageable [[wiki/index|index]] density while preserving semantic coherence.
...
For complex documents requiring both precision and context, implement a two-tier [[wiki/index|index]]:
...
At our current scale of ~1,200 chunks, performance is not a concern. We can safely increase chunk count by 3-5x without noticeable latency impact. Focus optimization efforts on chunk quality (content and metadata) rather than [[wiki/index|index]] tuning.
