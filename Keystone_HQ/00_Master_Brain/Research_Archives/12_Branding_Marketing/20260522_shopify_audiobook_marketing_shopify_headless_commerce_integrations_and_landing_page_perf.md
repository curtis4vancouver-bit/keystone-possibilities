...
For the Keystone Sovereign system, this architectural blueprint is mandatory. Shopify must be utilized exclusively as the transactional engine and identity provider. The autonomous AI agent system should rely on Shopify webhooks to register the purchase entitlement in an external database, while the actual audio files reside in an S3-compatible object storage solution, securely streamed to the client.   
...
app.get('/api/audio/:slug', async (c) => {
  ...
  try {
    ...
    // Setting Identity prevents Cloudflare from buffering for compression
