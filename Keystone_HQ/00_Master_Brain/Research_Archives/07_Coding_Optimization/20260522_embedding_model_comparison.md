...
### Critical Limitations for Keystone Brain
...
#### Nomic Embed v2 (2026)
- MoE architecture, ~137M parameters but activates fewer during inference.
...
| OpenAI text-embedding-3-large | 3072 (Matryoshka) | 8,191 | ~$0.13 | General-purpose API |
...
| Google Gemini Embedding 001 | 768 | 2,048 | Free tier available | General retrieval |

**Assessment for Keystone Brain:** API models introduce latency, network dependency, ongoing cost, and privacy concerns. Keystone Brain runs locally for a reason -- we process internal business documents, client data, and proprietary strategy. **API models are not recommended for the core brain**, but Voyage AI's code-3 model could be valuable for a secondary code-search index if needed.
...
For our sqlite-vec brain with potentially 100k+ documents, using 256-dim Matryoshka embeddings could reduce the vector index from ~300 MB to ~100 MB while maintaining excellent retrieval quality.
