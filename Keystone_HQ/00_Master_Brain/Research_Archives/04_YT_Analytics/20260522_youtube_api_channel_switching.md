...
```
Because Service Accounts represent an IAM identity in Google Cloud Platform rather than a real human user, they cannot have an associated YouTube channel. Therefore, **you must use the OAuth 2.0 User authorization flow** to obtain a User Access Token.
...
class YouTubeChannelManager:
    ...
    def verify_channel_details(self, channel_key):
        """Queries channel identity to verify selected brand channel matches target."""
