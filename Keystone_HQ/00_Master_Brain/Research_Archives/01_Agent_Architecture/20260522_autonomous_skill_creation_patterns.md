# Autonomous Skill Creation Patterns for AI [[AGENTS|Agents]]
...
Autonomous skill creation is the process by which AI [[AGENTS|agents]] transform successful task executions into reusable, portable procedural knowledge -- without human intervention. This report synthesizes research across open standards (agentskills.io), production frameworks (Hermes, Voyager, BabyAGI, Memento-Skills), and engineering best practices for skill extraction, validation, versioning, and deduplication. The key finding is that mature autonomous [[AGENTS|agents]] treat skills as first-class software artifacts: versioned, tested, deduplicated, and stored in searchable libraries for compositional reuse.

---

## 1. The SKILL.md Format and agentskills.io Standard

The `SKILL.md` format, published at **agentskills.io**, is the emerging open standard for portable agent skills. Originally introduced by Anthropic in late 2025 and released as an open standard on December 18, 2025, it works across Claude Code, OpenAI Codex, Cursor, GitHub Copilot, and Gemini CLI.
...
This is the critical performance optimization that makes large skill libraries practical:

| Stage | What Loads | When |
|:------|:-----------|:-----|
| **Discovery** | Only `name` and `description` from YAML frontmatter | At agent startup |
| **Activation** | Full `SKILL.md` markdown body | When user request matches skill description |
| **Execution** | Scripts, references, assets | Only if instructions require them |

This means an agent can have hundreds of installed skills without context window bloat. Only the ~100 tokens of name+description are loaded initially per skill.
...
The Hermes agent (Nous Research) implements a **closed learning loop** for autonomous skill extraction:
...
3. **Extract**: Agent autonomously writes a SKILL.md document capturing the procedure, pitfalls, and verification steps
4. **Store**: Skills saved to `~/.hermes/skills/` directory
5. **Retrieve**: On future similar tasks, the agent retrieves and loads the relevant skill document
6. **Refine**: During subsequent uses, if a more efficient approach is discovered, the agent updates the existing skill

### Key Implementation Details

- **Performance gain**: Skill reuse reportedly yields ~40% faster task completion vs. fresh reasoning
...
- **Active nudging**: The agent is programmed to periodically self-prompt: "Should I save this as a skill?" when it detects a recurring, automatable pattern
- **Compatibility**: Hermes skills follow the agentskills.io standard, making them portable

### Alongside Skill Memory

Hermes maintains three forms of persistent memory:
- `MEMORY.md`: General knowledge and learnings
- `USER.md`: User preferences and environment quirks
- `~/.hermes/skills/`: Procedural knowledge (the skill library)

---

## 3. The Antigravity Workflow-Skill-Creator Plugin

The `workflow-skill-creator` plugin (installed locally in the Keystone system) provides a structured, human-in-the-loop approach to skill distillation. Unlike fully autonomous extraction, it operates in **four phases**:
...
Produces an implementation plan covering name, directory structure, referenced skills, new scripts, rate limiting, and error handling. Requires explicit user approval.
...
Manual testing by invoking the agent with natural language that should trigger the new skill, plus running any sample query/answer pairs from brainstorming.

### Architectural Insight

The workflow-skill-creator represents a semi-autonomous pattern: the agent does the heavy lifting of distillation, but human judgment gates the process at Phase 1 (brainstorming) and Phase 2 (design approval). For fully autonomous systems, these gates would need to be replaced by self-evaluation heuristics.

---

## 4. Tool Call Counting Heuristics for Triggering Skill Extraction

When should an agent auto-extract a skill? The research reveals several practical heuristics:
...
- Below this threshold, the task is likely too simple to warrant skill creation
...
| Task recurrence | Same pattern seen 2+ times | Promote to skill |
| Backtracking detected | > 0 retries | Include failure-recovery in skill |
| Execution time | > 60 seconds | Candidate for optimization via skill |
| Confidence score | >= 0.5 relevance to existing skills | Load existing skill instead |
...
|:------|:-------------|:-------|
| **Component-Level** | Individual skill steps in isolation | Unit tests, mocked API calls |
...
1. **Task Success**: Does the skill resolve the intent within defined constraints (time, tool calls)?
...
Use a stronger, separate LLM to grade the auto-generated skill's outputs and reasoning against a rubric. This enables scalable assessment of qualitative goals like completeness, correctness, and style compliance. This is particularly useful when the skill was created autonomously without human brainstorming.

### Kill-Switch Controls

During staged rollouts, ensure clear rollback paths. If a new skill's performance degrades or it exhibits unpredictable behavior in canary mode, the system must be able to instantly revert to the previous version or disable the skill entirely.

---

## 6. Skill Versioning and Deduplication Strategies

### Versioning Best Practices

- **Treat skills as code**: Store in version control (Git). Each skill is a versioned directory
...
1. **Standardize**: Ensure each skill has uniform `name`, `description`, `tool_bindings`, `permissions` fields
2. **Embed**: Use sentence-transformers (e.g., `all-MiniLM-L6-v2`) to convert skill descriptions to dense vectors
...
Group skills into high-level categories (e.g., "Data Analysis", "API Integration", "File Processing"). The agent first selects the category, then the specific skill. This reduces the decision space and prevents the "decision-making fog" that occurs with too many flat, overlapping skills.

---

## 7. Real-World Examples of Autonomous Skill Creation

### Voyager (Minecraft Agent)

The seminal example of autonomous skill creation. Voyager uses GPT-4 to autonomously:
...
Explores self-building agent architecture:
...
- **Execute**: Run skill in sandbox
- **Reflect**: Analyze failures, attribute to specific weak skills
- **Write**: Optimize, rewrite, or create new skills

Key advantage: Deployment-time learning without model retraining. The external skill bank evolves while the base model remains stable.

### Hermes (Production Agent)

As detailed in Section 2, implements the most complete production-ready autonomous skill extraction with progressive disclosure, compatibility with agentskills.io, and active nudging for skill creation opportunities.
...
1. **Implement tool call counting**: Add a counter to agent sessions. When a successful task exceeds 5 tool calls, flag it for potential skill extraction
2. **Add a post-task reflection prompt**: After complex tasks, append: "This task involved N tool calls. Should I extract this as a reusable skill?"
3. **Use the existing workflow-skill-creator** for human-guided extraction; build autonomous extraction as a layer on top

### Medium-Term Architecture

4. **Build a skill deduplication pipeline**: Embed all existing skill descriptions, compute pairwise similarity, flag duplicates above 0.90 cosine threshold for review
5. **Implement semantic skill routing**: When a task arrives, embed the task description and retrieve top-k matching skills by cosine similarity before beginning execution
6. **Version skills in Git**: Each skill directory gets its own version in the YAML frontmatter; track changes via the brain vector DB

### Long-Term Vision

7. **Self-improving skill loop**: Implement the full Execute-Evaluate-Extract-Refine-Retrieve cycle
8. **Skill quality scoring**: Track skill usage frequency, success rate, and average time savings to rank and prune the skill library
9. **Cross-agent skill sharing**: Use the agentskills.io standard to share skills between Keystone sub-[[AGENTS|agents]]
...
- Anthropic documentation -- SKILL.md format and Claude Code integration
- Hermes / Nous Research -- Autonomous skill extraction workflow documentation
- Voyager (NVIDIA / MineDojo) -- Autonomous skill library creation in Minecraft
...
- arxiv.org -- Academic papers on skill extraction thresholds and relevance scoring
- Local Antigravity workflow-skill-creator plugin -- SKILL.md at C:\Users\Curtis\.gemini\config\plugins\science\skills\workflow_skill_creator\SKILL.md
