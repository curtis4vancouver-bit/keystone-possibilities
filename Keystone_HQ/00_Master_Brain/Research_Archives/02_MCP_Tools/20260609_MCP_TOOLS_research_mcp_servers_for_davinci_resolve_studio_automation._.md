...
Returns the timeline at the specified 1-based index (where 1 <= idx <= project.GetTimelineCount()).
...
Returns the master root folder of the Media Pool, the starting point for bin traversal.
...
Imprints a spatial marker at a specific frame index. The customData payload string is exceptionally valuable, frequently utilized by MCP servers to store JSON-encoded states or review notes from external LLMs.
...
GetNodeGraph() or GetNodeGraph(layerIndex) returns the Graph object associated with the clip. An optional layer index allows targeting specific nested grading passes.   
...
Applies a 1D or 3D LUT cube file to the specified node index. Requires a validated absolute filesystem path.
