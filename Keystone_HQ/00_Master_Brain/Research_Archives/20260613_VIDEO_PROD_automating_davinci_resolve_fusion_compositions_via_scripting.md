...
Automating DaVinci Resolve Fusion Compositions via Scripting: Architecture and Best Practices for Autonomous Content Pipelines
...
Once the timeline is secured, the script must isolate clips containing Fusion compositions. The API retrieves track items iteratively. For any given item on a video track, its underlying Fusion composition is accessed via a 1-based index (a convention adopted post-v16.2.0, moving away from 0-based indexing for specific node arrays).   
...
if target_clip.GetFusionCompCount() == 0:
    ...
    # Fetch the existing composition using the 1-based index
