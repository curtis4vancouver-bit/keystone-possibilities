...
Your ONLY job is to transcribe the user's voice input and send it to the agent on the screen using the `talk_to_antigravity(raw_transcript, structured_instruction)` tool.

RULES:
1. You MUST call the `talk_to_antigravity(raw_transcript, structured_instruction)` tool for ANY voice input, request, or comment from the user.
   - For `raw_transcript`, use the user's exact words, preserving personal style, context, and colloquialisms exactly as spoken.
   - For `structured_instruction`, provide a translated, step-by-step, low-ambiguity command based on the user's intent, formatted as a precise instruction designed for the coding agent.
...
def handle_f9_press():
    ...
    if state == STATE_SLEEPING:
        ...
        # Ignore toggle-press events that happen too close to activation to prevent bounce
        if now - activated_time < 0.8:
            print("[MIC] [F9 PRESS] Ignoring double-press toggle (too close to activation)")
            return

        if not speech_ended:
            # Stop recording / send audio end
            print("\n[MIC] [F9 PRESS] Stopping recording, processing speech...")
            with bridge.lock:
            ...
            # Force disconnect
            print("\n[MIC] [F9 PRESS] Force disconnecting...")
            bridge.deactivate()
...
def handle_f9_release():
    ...
    # If the key was held for more than 400ms, it is a hold (PTT mode).
...
def windows_keyboard_polling_worker():
    ...
    while not bridge.should_quit:
        ...
        if is_down:
            consecutive_release = 0
            consecutive_press += 1
            if consecutive_press >= 1 and not physical_f9_held:
                physical_f9_held = True
                handle_f9_press()
        else:
            consecutive_press = 0
            consecutive_release += 1
            if consecutive_release >= 1 and physical_f9_held:
                physical_f9_held = False
                handle_f9_release()
