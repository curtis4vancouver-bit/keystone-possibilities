        ...
        if (payload.referenceImageIds && Array.isArray(payload.referenceImageIds)) {
          payload.referenceImageIds.forEach(id => {
            if (id) {
              ...
              console.log(`[Flow Commander Inject] Attaching reference image via official action: ${cleanId}`);
              try {
                ...
                const matchedWorkflows = getWorkflows();
                const matchedWf = matchedWorkflows.find(w => w.id === cleanId || (w.metadata && w.metadata.primaryMediaId === cleanId));
                const name = matchedWf ? (matchedWf.title || matchedWf.metadata.displayName || "Image") : "Image";
                const url = matchedWf ? (matchedWf.thumbnailUrl || matchedWf.url || "") : "";

                const newIngredients = [...currentIngredients];
                newIngredients.push({
                  type: 'IMAGE',
                  ingredientId: cleanId,
                  imageId: cleanId,
                  name: name,
                  url: url,
                  addedTime: new Date(),
