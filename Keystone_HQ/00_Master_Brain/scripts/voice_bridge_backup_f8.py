...
def handle_f9_press():
    ...
    if state == STATE_SLEEPING:
        if bridge.activate("ptt"):
            print("\n[MIC] [F9 PRESS] Microphone activated. Streaming audio...")
...
def handle_f9_release():
    ...
    if state in (STATE_LISTENING, STATE_CONNECTING) and trigger == "ptt":
        if duration > 0.3:
            ...
            # Tap mode: check if it's a second tap to stop
            if press_time > activated_time + 0.2:
                if not speech_ended:
                    print(f"\n[MIC] [F9 TAP (TOGGLE off)] Stopping recording, processing...")
                    with bridge.lock:
                        bridge.speech_ended = True
                        bridge.speech_ended_time = now
                    bridge.gemini_queue.put(None)
            else:
                print(f"\n[MIC] [F9 TAP (TOGGLE on)] Microphone active. Tap F9 again to stop and send.")
                ...
                async def send_audio():
                    ...
                    while not bridge.should_quit:
                        ...
                        if not was_listening and not sentinel_sent:
                            was_listening = True
                            print("[send_audio] F9 active. Streaming started. Sending activityStart...")
                            try:
