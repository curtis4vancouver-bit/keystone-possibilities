...
> Master index of all Keystone Python systems. See also: Agent Fleet · Integration Map · Empire State
...
## Overnight Research
- **`overnight_research_daemon.py`** — Queue-driven Chrome Deep Research automation. Manages credit timing (30 prompts/5hr window), persists state to `credit_state.json`, generates morning reports. CLI: `--start`, `--dry-run`, `--status`, `--report`, `--next-batch`.
...
- **multiplexer_sync** — Syncs dynamic skills to the MCP multiplexer for hot-reloading.
...
- **`sync_daemon.py`** — Background sync daemon for keeping brain and fleet state consistent.
